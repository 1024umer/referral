            ...
            <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items-length="totalItems"
                ...
                @update:options="loadItems">
                    <template v-slot:item.is_active="{ item }">
                        <div :class="{ 'green-background': item.selectable.is_active === 1, 'red-background': item.selectable.is_active !== 1 }">
                            {{ item.selectable.is_active === 1 ? 'Yes' : 'No' }}
                        </div>
                    </template>
                    <template v-slot:item.is_featured="{ item }">
                        <div :class="{ 'green-background': item.selectable.is_featured === 1, 'red-background': item.selectable.is_featured !== 1 }">
                            {{ item.selectable.is_featured === 1 ? 'Yes' : 'No' }}
                        </div>
                    </template>
                    <template v-slot:item.actions="{ item }">
                        <!-- <v-btn link :to="{ name: 'auth.contacts.edit', params: { id: item.selectable.id } }" color="info" density="comfortable" size="small"
...
const itemtypeservice = new service('contacts')
