<template>
    <v-row>
        ...
        <v-col cols="1">
            <v-btn  color="success" link :to="{ name: 'auth.faqs.add' }">Add Faqs</v-btn>
        </v-col>
        <v-col cols="12">
            <v-row>
                <v-col>
        ...
        <div :class="{ 'green-background': item.selectable.is_active === 1, 'red-background': item.selectable.is_active !== 1 }">
            {{ item.selectable.is_active === 1 ? 'Yes' : 'No' }}
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn link :to="{ name: 'auth.faqs.edit', params: { id: item.selectable.id } }" color="info" density="comfortable" size="small"
