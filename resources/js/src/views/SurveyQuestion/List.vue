<template>
    <v-row>
        ...
        <v-col cols="1">
            <v-btn color="success" link :to="{ name: 'auth.surveyquestions.add' }">Add Questions</v-btn>
        </v-col>
        <v-col cols="12">
            <v-row>
                <v-col>
            ...
            <v-data-table-server v-model:items-per-page="itemsPerPage" :headers="headers" :items-length="totalItems"
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn link :to="{ name: 'auth.surveyquestions.edit', params: { id: item.selectable.id } }" color="info"
